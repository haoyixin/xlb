...
class ElementContainer<
 ...
 public:
  ...
  inline void reset(T new_value) {
    _value.store(new_value, butil::memory_order_seq_cst);
  }
  ...
  template <typename Op, typename T1>
  void modify(const Op &op, const T1 &value2) {
    T value = _value.load(butil::memory_order_consume);
    call_op_returning_void(op, value, value2);
    _value.store(value, butil::memory_order_release);
  }
